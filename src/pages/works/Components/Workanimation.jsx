...
const Workanimation = () => {
  ...
  return (
    <div className="bg-blue-900 text-white py-20 px-6 sm:px-12 relative overflow-hidden">
      ...
      {/* ══════════════════════════════════════
          DESKTOP ( md and above ) — unchanged
      ══════════════════════════════════════ */}
      <div className="hidden md:block">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12 items-center relative z-10">
          ...
          <div className="space-y-6 mt-[-100px]">
            ...
            <p className="text-lg text-blue-100 text-justify leading-relaxed ml-[-46px]">
        ...
        <div className="max-w-[1360px] mx-auto mt-16 p-6 bg-blue-900 squared-2xl border border-blue-600 shadow-md ml-[25px]">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-8 mb-10">
            {techCore.map((tech, idx) => (
              <div key={idx} className="flex flex-col items-center bg-blue-800 p-6 rounded-xl border border-blue-600 hover:shadow-lg transition">
      ...
      <div className="flex flex-col items-center gap-10 md:hidden">
        ...
        <div className="w-full text-center space-y-4">
          ...
          <p className="text-base text-blue-100 text-justify leading-relaxed">
            We're proud to deliver top-tier digital solutions. Our full project
            portfolio isn't public yet — reach out and we'll gladly show what
            we've crafted and how we can do the same for you.
          </p>
          <a
            href="/contact"
            className="inline-flex items-center gap-2 bg-yellow-400 text-blue-900 font-semibold py-3 px-6 hover:bg-yellow-300 transition"
          >
            <Mail size={18} /> Contact Our Team
          </a>
        </div>

        {/* Circle Form — centered, no side icons overlapping */}
        <div className="relative flex justify-center w-full">
          {/* Side icons — repositioned for mobile so they don't overlap */}
          <div className="absolute left-0 top-1/2 -translate-y-1/2 flex flex-col gap-3 z-20">
            <div className="bg-yellow-400 text-blue-900 p-3 rounded-full shadow-md">
              <Mail size={18} />
            </div>
            <div className="bg-green-400 text-blue-900 p-3 rounded-full shadow-md">
              <FaPhoneAlt size={18} />
            </div>
          </div>

          <div className="absolute right-0 top-1/2 -translate-y-1/2 flex flex-col gap-3 z-20">
            <div className="bg-orange-400 text-blue-900 p-3 rounded-full shadow-md">
              <MessageSquare size={18} />
            </div>
            <div className="bg-pink-400 text-blue-900 p-3 rounded-full shadow-md">
              <Send size={18} />
            </div>
          </div>

          {/* Circle */}
          <div className="relative w-[280px] h-[280px] bg-gradient-to-br from-blue-800 via-blue-900 to-blue-800 rounded-full border-[8px] border-orange-400 shadow-2xl flex items-center justify-center overflow-hidden">
            <div className="absolute top-8 left-1/2 transform -translate-x-1/2 z-20">
              <div className="w-[68px] h-[68px] bg-white text-blue-900 rounded-full flex items-center justify-center shadow-lg">
                <User size={32} />
              </div>
            </div>

            <form onSubmit={handleSubmit} className="w-[80%] space-y-3 text-blue-900 text-sm absolute z-10 pt-16">
              {step === 1 && (
                <input type="text" placeholder="Your Name" value={formData.name}
                  onChange={(e) => handleChange("name", e.target.value)}
                  className="w-full px-3 py-1.5 rounded-lg bg-blue-100 border border-gray-300 focus:outline-none focus:ring-2 focus:ring-yellow-400 text-sm" />
              ...
              <div className="flex justify-center gap-2 pt-1">
                {[1, 2, 3].map((s) => (
                  <div key={s} className={`w-2 h-2 rounded-full transition-all ${step >= s ? "bg-yellow-400" : "bg-blue-400"}`} />
                ))}
              </div>
              <div className="pt-1 flex justify-center">
                {step < 3 ? (
                  <button type="button" onClick={handleNext}
                    className="w-[90px] h-[40px] flex items-center justify-center text-white font-bold text-sm rounded-lg bg-gradient-to-br from-green-400 to-green-600 shadow-[0_4px_0_#2e7d32] hover:shadow-[0_2px_0_#2e7d32] active:translate-y-1 transition-all duration-200">
                    Next
                  </button>
                ) : (
                  <button type="submit"
                    className="w-[90px] h-[40px] flex items-center justify-center text-white font-bold text-sm rounded-lg bg-gradient-to-br from-yellow-400 to-yellow-600 shadow-[0_4px_0_#2e7d32] hover:shadow-[0_2px_0_#2e7d32] active:translate-y-1 transition-all duration-200">
                    Submit
                  </button>
                )}
              </div>
        ...
        {/* Tech Icons — 2 columns on mobile */}
        <div className="w-full mt-4 p-4 bg-blue-900 border border-blue-600 rounded-xl shadow-md">
          <h2 className="text-white text-lg font-semibold mb-5 text-center">Technologies We Use</h2>
          <div className="grid grid-cols-2 gap-4 mb-4">
            {techCore.map((tech, idx) => (
              <div key={idx} className="flex flex-col items-center bg-blue-800 p-4 rounded-xl border border-blue-600">
                <div className={`text-3xl mb-2 ${tech.color}`}>{tech.icon}</div>
                <p className="text-xs text-blue-100">{tech.label}</p>
              </div>
            ))}
          </div>
          <div className="grid grid-cols-2 gap-4">
            {techAdvanced.map((tech, idx) => (
              <div key={idx} className="flex flex-col items-center bg-blue-800 p-4 rounded-xl border border-blue-600">
                <div className={`text-3xl mb-2 ${tech.color || "text-white"}`}>{tech.icon}</div>
                <p className="text-xs text-blue-100">{tech.label}</p>
              </div>
            ))}
          </div>
